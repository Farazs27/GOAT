...
import { Calendar, Clock, User, MapPin, X, ChevronDown, CalendarCheck } from 'lucide-react';
...
function getMonthShort(d: string) {
  return new Date(d).toLocaleDateString('nl-NL', { month: 'short' }).toUpperCase();
}
...
export default function AppointmentsPage() {
  ...
  const visiblePast = showAllPast ? pastAppointments : pastAppointments.slice(0, 5);
  ...
  return (
    <div className="space-y-10">
      ...
      <div>
        ...
        <p className="text-base text-white/40">Bekijk en beheer uw aankomende en eerdere afspraken</p>
      </div>
        ...
        <>
          ...
          <section>
            <div className="flex items-center gap-3 mb-6">
              <h2 className="text-xl font-semibold text-white/85">Aankomende afspraken</h2>
              {upcomingCount > 0 && (
            ...
            {upcomingCount > 0 ? (
                      ...
                      <div className="flex-1 p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="space-y-3 flex-1">
                          ...
                          <div className="flex items-center gap-3 flex-wrap">
                            ...
                            <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${statusColors[appt.status] || 'bg-white/10 text-white/40'}`}>
                          ...
                          <div className="flex flex-wrap items-center gap-2.5">
                            <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-xl ${typeBadgeColors[appt.appointmentType] || 'bg-orange-500/15 text-orange-300'}`}>
                          ...
                          {appt.notes && (
                            <p className="text-xs text-white/30 mt-1">{appt.notes}</p>
                          )}
                        </div>
                          ...
                          <div className="shrink-0">
                            {confirmId === appt.id ? (
                              <div className="flex items-center gap-2">
                                ...
                                >
                                  {cancelling === appt.id ? 'Bezig...' : 'Bevestigen'}
                                </button>
                                <button
              ...
              >
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-sm text-white/35 mt-1">Geen aankomende afspraken</p>
                  <p className="text-xs text-white/20">Neem contact op met de praktijk om een afspraak te maken</p>
                </div>
              </div>
            )}
          </section>
          ...
          <section>
            <div className="flex items-center gap-3 mb-6">
              <h2 className="text-xl font-semibold text-white/50">Eerdere afspraken</h2>
            ...
            {pastAppointments.length > 0 ? (
              <div className="space-y-2.5">
                {visiblePast.map((appt: any, i: number) => (
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                      ...
                      <span className={`text-[11px] font-medium px-2.5 py-0.5 rounded-full shrink-0 ${statusColors[appt.status] || 'bg-white/5 text-white/30'}`}>
                        {statusLabels[appt.status] || appt.status}
                      </span>
                    </div>
                  </div>
                ...
                {pastAppointments.length > 5 && (
                  ...
                  >
                    <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-300 ${showAllPast ? 'rotate-180' : ''}`} />
                    {showAllPast ? 'Toon minder' : `Toon meer (${pastAppointments.length - 5})`}
                  </button>
                )}
              </div>
            ) : (
              ...
              >
                <p className="text-sm text-white/25">Geen eerdere afspraken gevonden</p>
              </div>
            )}
          </section>
        </>
