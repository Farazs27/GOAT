...
export async function POST(request: NextRequest) {
  try {
    ...
    // Run opmerkingen validation and enrich suggestions with warnings
    const enrichedSuggestions = enrichSuggestionsWithWarnings(
      suggestions,
      { patientAge: body.patientAge, recentCodes: body.recentCodes }
    );

    // Generate natural language summary (second Gemini call)
    const summaryInput = enrichedSuggestions.map(s => ({
      nzaCode: s.nzaCode,
    ...
    return Response.json({ response, suggestions: enrichedSuggestions });
  } catch (error) {
