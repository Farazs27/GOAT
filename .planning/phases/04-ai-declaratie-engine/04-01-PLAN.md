...
2. **VERIFY companion code logic** in `treatment-validator.ts`:
   - Confirm that `COMPANION_RULES` exists and defines auto-injection rules for A10 (verdoving), X10 (röntgen), etc.
   - Confirm that `validateAndCorrectSuggestions` includes a section (lines 333-370) that auto-adds missing companion codes.
   - Confirm that `isCompanion` field is set to `true` on injected codes (line 365).
   - This logic ALREADY EXISTS and works — no changes needed to treatment-validator.ts. This step is verification only.

3. Integrate into `treatment-chat/route.ts`:
...
4. Integrate into `analyze-notes/route.ts`:
  ...
  <verify>
    ...
    Grep for "COMPANION_RULES" in treatment-validator.ts to confirm companion logic exists.
    Grep for "isCompanion" in treatment-validator.ts to confirm companion field is set.
  </verify>
  <done>
    Both AI routes return suggestions/detected lines with an attached `warnings` array. Each warning has code, ruleType, severity, and Dutch message. At least 10 opmerkingen rules are implemented. Companion code auto-injection (A10, X10) confirmed working in treatment-validator.ts.
  </done>
</task>
