...
- GET: List patient's conversations ordered by updatedAt desc. Include: id, subject, status, practitioner name, lastMessage (content + createdAt + senderType), unreadCount (ConversationMessage where isRead=false and senderType=STAFF). Return array.
