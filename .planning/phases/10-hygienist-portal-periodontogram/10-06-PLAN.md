...
must_haves:
  truths:
    ...
    - "Perio chart data persists and shows line graph with color-coded BOP trends (HYG-02)"
    - "Dentist and hygienist notes appear in separate tabs with flag notifications (HYG-03)"
    - "Team Chat allows sending and receiving messages between staff members (HYG-05)"
  artifacts:
...
@.planning/phases/10-hygienist-portal-periodontogram/10-05-SUMMARY.md
@apps/web/src/app/(dashboard)/dashboard/page.tsx
</context>
...
<task type="auto">
  ...
  <files>apps/web/src/app/api/patient-portal/hygienist-notes/route.ts, apps/web/src/app/(patient)/portal/care-notes/page.tsx, apps/web/src/app/(patient)/portal/layout.tsx, apps/web/src/app/api/patients/[id]/odontogram/periodontal/route.ts</files>
  <action>
...
4. **Perio session auto-sync to clinical notes:**
   - Modify `apps/web/src/app/api/patients/[id]/odontogram/periodontal/route.ts` (the existing perio chart save route). After saving the chart, auto-create a HYGIENE type ClinicalNote containing:
