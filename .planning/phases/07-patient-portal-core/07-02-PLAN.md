...
**cancel/route.ts (verify existing, modify only if needed):**
- The cancel route already implements 24h rule and stores `cancelReason` (schema field exists). Verify it matches CONTEXT requirements:
  - 24h rule enforced (403 if < 24h)
  - `reason` field accepted in body and stored as `cancelReason`
  - Predefined reasons available: "Kan niet komen", "Voel me niet lekker", "Wil andere datum", "Andere reden"
- Only modify if existing implementation is missing any of these. Do NOT rewrite working code.
  </action>
