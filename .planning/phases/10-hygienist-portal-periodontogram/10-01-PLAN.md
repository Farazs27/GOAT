...
must_haves:
  truths:
    - "HYGIENIST role user sees filtered sidebar without Smile Design and Settings"
...
2. In `apps/web/src/app/(dashboard)/layout.tsx`:
   ...
   - Create an inline or co-located client component `RoleNav` that:
     ...
     - Defines `HYGIENIST_HIDDEN = ['/smile-design', '/settings']` (only these two — hygienist retains full AI access per user decision)
  ...
  <verify>Run `pnpm --filter @dentflow/web build` — no errors. Log in as hygienist (hygienist@tandarts-amsterdam.nl / Welcome123) — sidebar should NOT show Smile Design or Settings. AI Logs should remain visible. Log in as dentist — all items visible.</verify>
  <done>PerioToothData has new fields with backward compatibility. Hygienist sees filtered sidebar (no Smile Design, no Settings). AI access retained. Dentist sidebar unchanged.</done>
</task>
...
- Hygienist login shows filtered nav (no Smile Design, no Settings — AI Logs remains visible)
